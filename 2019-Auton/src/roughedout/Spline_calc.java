...
public class Spline_calc {
	public static class SplineInterpolator {
		
		private final List<Float> mX;														//list of x coordinates
		private final List<Float> mY;														//list of y coordinates
		...
		public static SplineInterpolator createMonotoneCubicSpline(List<Float> x, List<Float> y) {
			...
			for (int i = 0; i < n - 1; i++) {
				...
				if (h <= 0f) {throw new IllegalArgumentException("The control points must all have strictly increasing X values.");}
				d[i] = (y.get(i + 1) - y.get(i)) / h;
			}
			
			m[0] = d[0];																	// Initialize the tangents as the average of the secants.
			for (int i = 1; i < n - 1; i++) {
				m[i] = (d[i - 1] + d[i]) * 0.5f;
			}
			m[n - 1] = d[n - 2];
			...
			return new SplineInterpolator(x, y, m);
																							 ...
																							 */
		public float interpolate(float x) {													// Handle the boundary cases.
			final int n = mX.size();
			if (Float.isNaN(x)) {
				return x;
			}
